...
class SolrSearch(object):
    option_modules = ('query', 'filters', 'paginator', 'more_like_this',
                      'highlighter', 'faceter', 'sorter', 'facet_querier')
    def __init__(self, interface, original=None):
        self.interface = interface
        self.schema = interface.schema
        if original is None:
            self.query = LuceneQuery(self.schema, 'q')
            self.filters = LuceneQuery(self.schema, 'fq')
    ...
    def filter(self, *args, **kwargs):
        newself = self.clone()
        newself.filters.add(args, kwargs)
        return newself
    ...
    def __getitem__(self, k):
        if isinstance(k, slice):
            return self.paginate(k.start, k.stop)
        else:
            return self.paginate(k, k)
    ...
    def boost_relevancy(self, boost_score, **kwargs):
        ...
        newself = self.clone()
        newself.query.add_boost(kwargs, boost_score)
        return newself
